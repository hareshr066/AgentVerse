...
def sync_neon_to_engine(engine, agent_type: str):
    ...
    with engine.begin() as conn:
        if agent_type == "inventory":
            rows = fetch_neon_table("inventories")
            for r in rows:
                conn.execute(
                    text("""
                    INSERT INTO inventories (product_name, current_stock, average_daily_usage, lead_time, safety_stock, reorder_point, eoq, status)
                    VALUES (:product_name, :current_stock, :average_daily_usage, :lead_time, :safety_stock, :reorder_point, :eoq, :status)
                    ...
                    """),
                    {
                        ...
                        "average_daily_usage": r.get("average_daily_usage", 0.0),
                        "lead_time": r.get("lead_time", 0),
                        "safety_stock": r.get("safety_stock", 0.0),
                        "reorder_point": r.get("reorder_point", 0.0),
                        "eoq": r.get("eoq", 0.0),
                        "status": r.get("status", "IN_STOCK")
                    }
                )
